...
export default {
	...
	async getSubreddit(context, payload) {
		...
		console.log('Mocking getSubreddit for', payload.subredditName);
		await new Promise((resolve) => setTimeout(resolve, 300));

		// Mock data
		const mockSubreddit = {
			title: payload.subredditName,
			nickname: payload.subredditName,
			picture: '',
			banner: '',
			isMember: true,
			subredditId: 'mock-subreddit-id',
			isModerator: true,
			isFavorite: false,
			members: 1234,
			dateOfCreation: new Date().toISOString(),
			type: 'Public',
			description: 'This is a mock subreddit description.',
			mainTopic: 'Data',
			subTopics: ['Analysis', 'Visualization'],
			nsfw: false,
		};

		context.commit('setSubreddit', mockSubreddit);
	},
	/**
